...
class TestL9859(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(self.analysis.filepath, "datasets/cleaned_data_20240531.pickle")
        ...
        self.assertIsInstance(self.analysis.polychord_settings, dict, "Polychord settings should be a dictionary.")
        self.assertIn("do_clustering", self.analysis.polychord_settings, "Clustering setting should be in polychord settings.")
        
        # Verify output directory parameter
        self.assertEqual(self.analysis.output_params["base_dir"], "output_dir/specific_run",
    ...
    def test_load_data(self, mock_unpickle_data):
        ...
        mock_unpickle_data.return_value = {
            "ESPRESSO_pre": {
                "RV": [], "Time": [], "FWHM": [], "e_FWHM": [], "Sindex": [], "e_Sindex": []
            },
            "ESPRESSO_post": {
                "RV": [], "Time": [], "FWHM": [], "e_FWHM": [], "Sindex": [], "e_Sindex": []
            },
        ...
        self.assertIsNotNone(self.analysis.X_pre, "X_pre should not be None after loading data.")
        self.assertIsNotNone(self.analysis.X_post, "X_post should not be None after loading data.")
        self.assertIsNotNone(self.analysis.X_harps, "X_harps should not be None after loading data.")
    ...
    def test_load_data_failure(self, mock_unpickle_data):
        ...
        with self.assertRaises(IOError, msg="Should raise IOError if the data file is missing"):
    ...
    def test_initialise_parameters_default(self):
        ...
        self.assertNotIn("A_FWHM", self.analysis.parameters, "A_FWHM should not be in parameters")
        self.assertNotIn("A_Sindex", self.analysis.parameters, "A_Sindex should not be in parameters")
        
        # Check the total number of parameters and derived parameters
        self.assertEqual(len(self.analysis.parameters), 22, "Total parameters should match the expected count without features")
        self.assertEqual(self.analysis.nDims, len(self.analysis.parameters), "nDims should match the number of parameters")
        self.assertEqual(self.analysis.nDerived, 2, "nDerived should match expected count for base features only")
    ...
    def test_initialise_parameters_full(self):
        ...
        self.assertIn("A_RV", self.analysis.parameters, "A_RV should be in parameters")
        self.assertIn("A_FWHM", self.analysis.parameters, "A_FWHM should be in parameters")
        self.assertIn("A_Sindex", self.analysis.parameters, "A_Sindex should be in parameters")
        
        # Check the total number of parameters and derived parameters
        self.assertEqual(len(self.analysis.parameters), 40, "Total parameters should match the expected count with all features")
        self.assertEqual(self.analysis.nDims, len(self.analysis.parameters), "nDims should match the number of parameters")
        self.assertEqual(self.analysis.nDerived, 3, "nDerived should match expected count with all features enabled")
    ...
    def test_derive_params(self):
        ...
        self.analysis.Q = {
            "secosw_b": 0, "sesinw_b": 1,
            "secosw_c": 2, "sesinw_c": 3,
            "secosw_d": 4, "sesinw_d": 5,
        }
        q_values = [0.6, 0.8, 0.3, 0.4, 0.5, 0.5]  # Mock values for secosw, sesinw for planets b, c, d
        ...
        if self.analysis.include_planet_b:
            self.assertAlmostEqual(self.analysis.e_b, expected_e_b, "Eccentricity of planet b does not match expected value")
        self.assertAlmostEqual(self.analysis.e_c, expected_e_c, "Eccentricity of planet c does not match expected value")
        self.assertAlmostEqual(self.analysis.e_d, expected_e_d, "Eccentricity of planet d does not match expected value")
    ...
    def test_compute_planets_RV(self, mock_params, mock_rv_model):
        ...
        mock_rv_model_instance.return_value = np.random.normal(0, 1, len(self.T0))  # Mock radial velocity data
        ...
        self.assertIsInstance(rv_total, np.ndarray, "Returned RV data should be a numpy array")
        self.assertEqual(len(rv_total), len(self.T0), "Length of RV data should match length of input times")
    ...
    def test_planet_prior(self, mock_uniform, mock_kipping_beta, mock_gaussian):
        ...
        mock_uniform.side_effect = lambda x, low, high: (low + high) / 2  # Returns midpoint

        # Initialise parameter values for testing
        q_vals = np.full(len(self.analysis.parameters), 0.5)  # Base value for simplicity
        transformed_qq = self.analysis.planet_prior(q_vals.copy())

        # Check that the transformed values are as expected
        if self.analysis.include_planet_b:
            self.assertAlmostEqual(transformed_qq[self.analysis.Q["P_b"]], 2.2531136, places=5)
        self.assertAlmostEqual(transformed_qq[self.analysis.Q["P_c"]], 3.6906777, places=5)
        self.assertAlmostEqual(transformed_qq[self.analysis.Q["secosw_c"]], 0.2, places=5)
        self.assertAlmostEqual(transformed_qq[self.analysis.Q["w_c"]], 0, places=5)  # Uniform distribution centered at 0
    ...
    def test_myprior(self, mock_gaussian, mock_jeffreys, mock_uniform):
        ...
        for param, index in self.analysis.Q.items():
            if index in expected_values:
                self.assertAlmostEqual(transformed_qq[index], expected_values[index], places=5,
    ...
    def test_mean_fxn(self):
        ...
        if self.analysis.include_fwhm:
            self.assertEqual(results[1].shape, self.analysis.adjusted_time_FWHM.shape, "Y1 results shape mismatch.")
        else:
            self.assertIsNone(results[1], "Y1 should be None when FWHM is not included.")
        if self.analysis.include_sindex:
            self.assertEqual(results[2].shape, self.T0.shape, "Y2 results shape mismatch.")
        else:
            self.assertIsNone(results[2], "Y2 should be None when Sindex is not included.")
