...
def myprior(q):
    ...
    qq[Q["t_decay"]] = pt.jeffreys(
        q[Q["t_decay"]], qq[Q["P_rot"]] / 2, 2600
    )  # T_decay > P_rot/2 + J(2.5, 2600)
    qq[Q["gamma"]] = pt.uniform(q[Q["gamma"]], 0.05, 5)  # U(0.05, 5)
    qq[Q["sigma_RV_pre"]] = pt.uniform(
        q[Q["sigma_RV_pre"]], 0, max_jitter_pre
    )  # U(0, max_jitter_pre)
    qq[Q["sigma_RV_post"]] = pt.uniform(
        q[Q["sigma_RV_post"]], 0, max_jitter_post
    )  # U(0, max_jitter_post)
...
def myloglike(theta):
    ...
    K_trial = (
        q[Q["A_RV"]] ** 2
        * kernels.ExpSine2Kernel(gamma=q[Q["gamma"]], log_period=log_period)
        * kernels.ExpSquaredKernel(metric=q[Q["t_decay"]] ** 2)
    )
...
output_params = {"base_dir": output_dir, "feedback": 0}
